...
if __name__ == "__main__":
    ...
    l_chunks = int(nblocks/size) #average length of each chunk = number of HDF5 blocks per chunk 
    ...
    for iblock in range(istart,istart+int(l_chunks_arr[rank])):
        ...
        if iblock < istart+l_chunks: iii = iblock
        if iblock == istart+l_chunks: iii = n_chunks*l_chunks+rank # this only exists for a chunk that received a leftover
