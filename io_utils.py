...
class CartesianGrid:
    ...
    def __init__(
        self, L=1.0, n_x=1, n_y=1, n_z=1, time=1.0, hubble=1.0, omega0=0.3, omegaL=0.7
    ):
        self.L = L
        self.n_x = n_x
        self.n_y = n_y
...
def read_block_vars(fname, ptype):
    if ptype == 0:
        print("Processing GAS PARTICLES")
    if ptype == 1:
        print("Processing DM PARTICLES")
    if ptype == 4:
        print("Processing STARS+WIND PARTICLES")
    if ptype == 5:
        print("Processing BH PARTICLES")
    # position
